...
partial class ConsensusService
{
    private bool CheckPrepareResponse(uint pId)
    {
        if (context.TransactionHashes[pId]!.Length == context.Transactions[pId]!.Count)
        {
            ...
            localNode.Tell(new LocalNode.SendDirectly(context.MakePrepareResponse(pId)));
    ...
    {
        if (context.PreparationPayloads.Count(p => p != null) >= context.M && context.TransactionHashes[pId]!.All(p => context.Transactions[pId]!.ContainsKey(p)))
