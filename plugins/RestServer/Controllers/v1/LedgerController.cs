...
public class LedgerController : ControllerBase
{
        ...
        int take = 50)
    {
        ...
        var accounts = NativeContract.Governance.ListGasAccounts(_neoSystem.StoreView, _neoSystem.Settings);
        ...
        int take = 50)
    {
        if (skip < 1 || take < 1 || take > RestServerSettings.Current.MaxPageSize)
            throw new InvalidParameterRangeException();
        var accounts = NativeContract.Governance.ListNeoAccounts(_neoSystem.StoreView, _neoSystem.Settings);
        if (accounts.Any() == false)
            return NoContent();
        var accountsList = accounts.OrderByDescending(o => o.Balance).Skip((skip - 1) * take).Take(take);
        if (accountsList.Any() == false)
