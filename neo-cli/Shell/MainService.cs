        ...
        private bool OnShowStateCommand(string[] args)
        {
            ...
            Task task = Task.Run(async () =>
            {
                while (!stop)
                {
                    ...
                    WriteLineWithoutFlicker($"block: {wh}/{Blockchain.Singleton.StateHeight}/{Blockchain.Singleton.Height}/{Blockchain.Singleton.HeaderHeight}  connected: {LocalNode.Singleton.ConnectedCount}  unconnected: {LocalNode.Singleton.UnconnectedCount}");
                    int linesWritten = 1;
